...
Conversation images use authenticated dashboard media reads. Hermes `MEDIA:` paths are recognized only in settled assistant output outside Markdown code, with up to four rendered per message. The active profile and credential scope each bounded 16 MiB in-memory image response, and disk caching is disabled. Failed loads offer explicit Retry.
