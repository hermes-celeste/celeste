...
Conversation images use explicit HTTPS Markdown sources or authenticated dashboard media reads. Hermes `MEDIA:` paths are recognized only in assistant output, fetched through the configured dashboard with the active credential, decoded only for image media types, and bounded to 16 MiB.
