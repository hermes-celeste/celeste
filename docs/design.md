...
- Conversation text and composer text share the compact `bodyMedium` scale. The composer, drawer control, Send action, Stop action, and jump-to-latest action use a consistent compact geometry with borderless tonal surfaces.
- Sending a message opts into following the latest transcript content. Scrolling upward pauses automatic following during streaming; tapping the centered jump-to-latest control or manually reaching the bottom resumes it. The standalone control floats above the composer whenever content continues below the viewport.
...
- Routine reasoning and tool activity form chronological segments around assistant commentary. The transcript presents each activity segment as a compact **Thinking** entry with a restrained three-beat activity pulse that opens the mobile **Steps** sheet. High-salience interactions such as approvals and user questions retain their dedicated surfaces.
- Completed background work appears as a quiet terminal icon and **Background process** label. It opens the shared mobile inspection sheet for selectable command, semantic completion status, exit code, and bounded monospaced output.
